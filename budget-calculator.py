...
def export_to_markdown(results: list[dict], filename: str = "budget-comparison.md") -> None:
    ...
    with open(filename, "w") as f:
        ...
        for r in results:
            if r["total"] > 0:
                f.write(
                    f"| {r['city']} | {format_currency(r['total'])} | {r['visa_score']}/5 | {r['flight_coverage']}% |\n"
                )
...
def analyze_single_location(data: dict, city_name: str) -> None:
    ...
    for location in data.get("locations", []):
        if location.get("city", "").lower() == city_name.lower():
            ...
            print(f"\n{'Per-Person Costs':^40}")
            print("-" * 40)
            per_person_flight = costs["flights"] / team_size
            per_person_hotel = costs["hotels"] / team_size
            per_person_other = (costs["venue"] + costs["catering_estimate"] + costs["ground_transport"]) / team_size

            print(f"  Flight avg:     {format_currency(per_person_flight):>12}")
            print(f"  Hotel:          {format_currency(per_person_hotel):>12}")
            print(f"  Other:          {format_currency(per_person_other):>12}")
